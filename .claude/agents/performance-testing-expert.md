...
function orderWorkflow(token) {
  ...
  // Create order
  const createOrderResponse = http.post(`${BASE_URL}/api/orders`, JSON.stringify({}), {
    headers,
  ...
  if (createOrderResponse.status === 201) {
    ...
    const addItemResponse = http.post(
      `${BASE_URL}/api/orders/${order.id}/items`,
      JSON.stringify({
        productId: '1',
    ...
    if (Math.random() < 0.3) {
      const confirmResponse = http.post(
        `${BASE_URL}/api/orders/${order.id}/confirm`,
...
class PerformanceMonitor {
  ...
  getHttpMetricsMiddleware() {
    return (req, res, next) => {
      ...
      res.on('finish', () => {
        ...
        this.httpRequestTotal.labels(req.method, route, res.statusCode.toString()).inc();
      });
  ...
  recordDatabaseQuery(operation, table, duration) {
    this.databaseQueryDuration.labels(operation, table).observe(duration / 1000); // Convert to seconds
  }
