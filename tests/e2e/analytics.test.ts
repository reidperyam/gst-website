...
test.describe('Google Analytics E2E Tests', () => {
  ...
  test.describe('CTA Tracking', () => {
    ...
    test('should track hero primary CTA button click (Calendly)', async ({ page }) => {
      await gotoAndSetupAnalytics(page, '/');

      // Prevent navigation to external site
      await page.route('**/calendly.com/**', route => route.abort());

      // Find and click hero primary CTA button
      const heroCTA = page.locator('a.cta-button.primary[href*="calendly.com"]').first();
      await expect(heroCTA).toBeVisible();

      await heroCTA.click();

      // Verify cta_click event was tracked with correct parameters
      const events = await page.evaluate(() => (window as any).gtagEvents || []);
      const ctaEvent = events.find((e: any) => e.eventName === 'cta_click');
      expect(ctaEvent).toBeDefined();
      expect(ctaEvent?.eventData.cta_type).toBe('calendly');
      expect(ctaEvent?.eventData.location).toBe('hero');
      expect(ctaEvent?.eventData.event_category).toBe('engagement');
    });

    test('should track hero secondary CTA button click (Services)', async ({ page }) => {
      await gotoAndSetupAnalytics(page, '/');

      // Find and click hero secondary CTA button
      const heroSecondary = page.locator('a.cta-button.secondary[href="/services"]').first();
      await expect(heroSecondary).toBeVisible();

      // Note: Don't prevent navigation, let it navigate to services
      ...
      const ctaEvent = events.find((e: any) => e.eventName === 'cta_click');
      expect(ctaEvent).toBeDefined();
      expect(ctaEvent?.eventData.cta_type).toBe('services');
      expect(ctaEvent?.eventData.location).toBe('hero');
      expect(ctaEvent?.eventData.event_category).toBe('engagement');
    });

    test('should track both hero CTAs in sequence', async ({ page }) => {
      await gotoAndSetupAnalytics(page, '/');
      ...
      const heroPrimary = page.locator('a.cta-button.primary[href*="calendly.com"]').first();
      await expect(heroPrimary).toBeVisible();
      await heroPrimary.click();

      // Verify first CTA event
      let events = await page.evaluate(() => (window as any).gtagEvents || []);
      let ctaEvents = events.filter((e: any) => e.eventName === 'cta_click');
      expect(ctaEvents.length).toBeGreaterThanOrEqual(1);
      expect(ctaEvents[0]?.eventData.cta_type).toBe('calendly');

      // Click hero secondary CTA (go back first to reset)
      await page.goto('/');
      await setupAnalyticsMocking(page);
      ...
      // Verify second CTA event
      events = await page.evaluate(() => (window as any).gtagEvents || []);
      ctaEvents = events.filter((e: any) => e.eventName === 'cta_click');
      expect(ctaEvents.length).toBeGreaterThanOrEqual(1);
      expect(ctaEvents[ctaEvents.length - 1]?.eventData.cta_type).toBe('services');
    });
  });
