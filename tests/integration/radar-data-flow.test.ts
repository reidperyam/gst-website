...
class RadarDataFlowSimulator {
  ...
  processFyi(annotatedItems: InoreaderItem[]): RadarFyiItem[] {
    return annotatedItems.map(toFyiItem).filter((item): item is RadarFyiItem => item !== null);
  }

  /**
   * Process raw items into balanced, deduplicated Wire display models.
   */
  processWire(rawItems: InoreaderItem[], fyi: RadarFyiItem[]): RadarWireItem[] {
...
describe('Radar Data Flow', () => {
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle items with no URL gracefully in dedup', () => {
      const rawItems = [makeItem({ canonical: undefined, alternate: undefined })];
  ...
  describe('Full Pipeline', () => {
    it('should process a realistic mixed feed correctly', () => {
      ...
      // Chronological order
      for (let i = 1; i < wire.length; i++) {
        expect(new Date(wire[i - 1].publishedAt).getTime()).toBeGreaterThanOrEqual(
          new Date(wire[i].publishedAt).getTime()
        );
      }
    ...
    it('should produce a correctly merged unified feed', () => {
      ...
      for (let i = 1; i < feed.length; i++) {
        expect(new Date(feed[i - 1].sortDate).getTime()).toBeGreaterThanOrEqual(
          new Date(feed[i].sortDate).getTime()
        );
      }
